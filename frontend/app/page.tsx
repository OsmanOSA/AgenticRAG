    ...
    <>
      ...
      <aside className={cn(
        ...
        <div className="px-4 shrink-0">
          <div className="rounded-2xl bg-white border border-[#e0e0e0] p-4 shadow-sm">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-medium text-[#5f6368] uppercase tracking-wider">Index</span>
              <div className={cn("size-2 rounded-full", status?.ready ? "bg-[#34a853]" : "bg-[#dadce0]")} />
            </div>
            <div className="text-2xl font-semibold text-[#1f1f1f] tabular-nums">
              {status?.point_count ?? "—"}
            </div>
            <div className="text-xs text-[#80868b] mt-0.5">{status?.collection ?? "…"}</div>
            <button
              onClick={onIngest}
              disabled={ingesting}
              className="mt-3 w-full flex items-center justify-center gap-2 px-3 py-2 rounded-xl bg-[#f8f9fa] hover:bg-[#e8eaed] text-sm text-[#5f6368] hover:text-[#1f1f1f] transition-all disabled:opacity-50"
            >
