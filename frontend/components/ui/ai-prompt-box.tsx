  ...
  return (
    <>
      <PromptInput
        ...
        className={cn("w-full bg-[#1F2023] border-[#444444] shadow-[0_8px_30px_rgba(0,0,0,0.24)] transition-all duration-300 ease-in-out", isRecording && "border-red-500/70", className)}
        ...
        <PromptInputActions className="flex items-center justify-between gap-2 p-0 pt-2">
          <div className={cn("flex items-center gap-1 transition-opacity duration-300", isRecording ? "opacity-0 invisible h-0" : "opacity-100 visible")}>
            ...
            <div className="flex items-center">
              {[
                { id: "search", icon: <Globe className="w-4 h-4" />, label: "Search", activeColor: "#1EAEDB", activeBg: "bg-[#1EAEDB]/15", activeBorder: "border-[#1EAEDB]", active: showSearch },
                { id: "think", icon: <BrainCog className="w-4 h-4" />, label: "Think", activeColor: "#8B5CF6", activeBg: "bg-[#8B5CF6]/15", activeBorder: "border-[#8B5CF6]", active: showThink },
                { id: "canvas", icon: <FolderCode className="w-4 h-4" />, label: "Canvas", activeColor: "#F97316", activeBg: "bg-[#F97316]/15", activeBorder: "border-[#F97316]", active: showCanvas },
              ].map((item, i) => (
                <React.Fragment key={item.id}>
                  {i > 0 && <CustomDivider />}
                  <button
                    type="button"
                    onClick={() => item.id === "canvas" ? setShowCanvas((p) => !p) : handleToggleChange(item.id)}
                    className={cn(
                      "rounded-full transition-all flex items-center gap-1 px-2 py-1 border h-8",
                      item.active ? `${item.activeBg} ${item.activeBorder}` : "bg-transparent border-transparent text-[#9CA3AF] hover:text-[#D1D5DB]"
                    )}
                    style={item.active ? { color: item.activeColor } : undefined}
                  >
                    <div className="w-5 h-5 flex items-center justify-center flex-shrink-0">
                      <motion.div
                        animate={{ rotate: item.active ? 360 : 0, scale: item.active ? 1.1 : 1 }}
                        whileHover={{ rotate: item.active ? 360 : 15, scale: 1.1, transition: { type: "spring", stiffness: 300, damping: 10 } }}
          ...
          <PromptInputAction tooltip={isLoading ? "Stop generation" : isRecording ? "Stop recording" : hasContent ? "Send message" : "Voice message"}>
            <Button
              ...
              className={cn(
                ...
                isRecording ? "bg-transparent hover:bg-gray-600/30 text-red-500" : hasContent ? "bg-white hover:bg-white/80 text-[#1F2023]" : "bg-transparent hover:bg-gray-600/30 text-[#9CA3AF]"
              )}
              onClick={() => {
